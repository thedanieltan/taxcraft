...
const [registry, familyCatalog] = await Promise.all([
...
assert(
  registry.scope.entityCount === registry.entities.length,
  "Declared PIT jurisdiction count does not match the register",
);
...
for (const jurisdiction of resolved) {
  ...
  assert(
    implementationStatuses.has(jurisdiction.implementationStatus),
    `${jurisdiction.code} has an invalid implementation status`,
  );
  assert(
    familyIds.has(jurisdiction.calculationFamily),
    `${jurisdiction.code} references an unknown calculation family`,
  );

  if (jurisdiction.mappingStatus === "unmapped") {
    assert(
      jurisdiction.calculationFamily === "UNMAPPED",
      `${jurisdiction.code} is unmapped but declares a calculation family`,
    );
  }
...
for (const requiredCode of ["GB", "SG"]) {
  const jurisdiction = resolved.find(({ code }) => code === requiredCode);
  assert(jurisdiction, `Missing existing jurisdiction ${requiredCode}`);
